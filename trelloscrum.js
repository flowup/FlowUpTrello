...
function computeTotal(){
	...
	ctto = setTimeout(function(){
		...
		for (var i in _pointsAttr){
			var score = 0, cash = 0,
				attr = _pointsAttr[i];
			$('#board :not(.list-title-special-card) .list-total:not(.list-total-cash) .'+attr).each(function(){
				score+=parseFloat(this.textContent)||0;
			});
			var scoreSpan = $('<span/>', {class: attr}).text(round(score)||0);
			$total.append(scoreSpan);

			$('#board :not(.list-title-special-card) .list-total.list-total-cash .'+attr).each(function(){
